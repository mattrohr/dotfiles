...
## Installation
1. (_macOS only_) Log into Mac App Store so macOS apps specified in [`Brewfile`](./Brewfile) are installed.
...
## Notes
If `.secret` exists, it will be sourced in [`.zshrc`](./.zshrc) along with the other files. You may use this to add commands, files, and directories you don’t want to commit to a public repository like `~/.ssh/config`, `/etc/hosts`, and `~/.gnupg/`
